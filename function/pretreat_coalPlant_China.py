...
def pretreat_coalPlant_GEM():
    filePath = 'input/RawData/Global-Coal-Plant-Tracker-January-2024.xlsx'
    resolution = 20 # km
    pp_data_GEM = pd.read_excel(filePath, sheet_name='Units')
    columnName = 'Value'
    
    # status_filter = ['announced', 'operating', 'construction', 'permitted', 'pre-permit']
    status_filter = ['operating', 'construction', 'permitted']
    ...
    pp = pd.concat([pp_withYrs, pp_2, pp_5], axis=0)
    ...
    pp['pixelY'] = round((55 - pp['lat']) / 0.01 / resolution)
    pp = pp.set_index(pp['ID'])
    
    # find the nearest carbon storage point and calculate the distance
    ccs_point = pd.read_excel('input/RawData/ccs_clusters.xlsx')
    for p in pp.index:
        lon_p = pp.loc[p, 'lon']
        lat_p = pp.loc[p, 'lat']
    ...
    if len(tmp1)>0:
        tmp1['option'] = 'CCS'
        tmp1['E2020'] = tmp1.apply(lambda x: cal_emiss_yr(0,0,1,0,0,1,x['Capacity_kw'], x['hours'],0,0,x['CoalUnit_g/kwh'], x['year'], 2020), axis=1)
        tmp1['ccs'] = [(int(1), int(x)) for x in tmp1['year']]
        tmp2 = pp[pp['Combustion technology'] != 'ultra-supercritical/ccs']
        tmp2['option'] = 'PP'
        tmp2['E2020'] = tmp2.apply(lambda x: cal_emiss_yr(0,0,0,0,0,1,x['Capacity_kw'], x['hours'],0,0,x['CoalUnit_g/kwh'], x['year'], 2020), axis=1)
    ...
    # pretreat
    pp['take'] = 0.
    pp = pp.rename(columns={'Subnational unit (province, state)':'prov'})
    
    # merge unit to plant
    key_info = ['hours', 'CoalUnit_g/kwh', 'lon', 'lat', 'year', 
                'pixelX','pixelY','ccsDis_km', 'option', 'pp',
                'flex','be','ccs','retire','take', 'prov']
...
def pretreat_coalPlant_China():
    
    # ======== 1. pretreat coal plant dataset ================
    filePath = 'input/RawData/pp_lhr.xlsx'
    pp_data_ori = pd.read_excel(filePath, sheet_name = 'Sheet1')
    pick_col = ['CCPUD20220101', 'NAME','PROV', 'YEAR', 'capacity','status_201707',
                'latitude','Longitude','HOURS','gce/kwh','Boiler_type']
    pp_data = pp_data_ori.loc[:,pick_col]
    
    # filter the inputYrs > 1985
    pp_data = pp_data[pp_data['YEAR'] > 1985]
    
    # filter the status and fillNan
    status_pick = ['opearting', 'operating', 'construction_201707',
                   'construction_201807', 'operating_201712', 'permitted',
                   'announced', 'PLN', 'operating ', 'pre-permit', 'operating_201707', 
                   'construction_201712', 'construction', 'CON', 'RET']
    
    pp_data = pp_data[pp_data['status_201707'].isin(status_pick)]
    
    pp_noYear_index = pp_data[pp_data['YEAR'] == 9999].index
    pp_data.loc[pp_noYear_index,'YEAR'] = 2025
    
    # merge units to plant
    pp_data.fillna('null', inplace=True)
    
    # fill coal emission based on Boiler_type
    index1 = pp_data[(pp_data['Boiler_type'] == 'Ultra-super') & (pp_data['capacity'] >= 600 & (pp_data['gce/kwh'] == 'null') )].index
    pp_data.loc[index1, 'gce/kwh'] = 285
    index1 = pp_data[(pp_data['Boiler_type'] == 'Ultra-super') & (pp_data['capacity'] < 600 & (pp_data['gce/kwh'] == 'null') )].index
    pp_data.loc[index1, 'gce/kwh'] = 293    
    index1 = pp_data[(pp_data['Boiler_type'] == 'Super') & (pp_data['capacity'] >= 300 & (pp_data['gce/kwh'] == 'null') )].index
    pp_data.loc[index1, 'gce/kwh'] = 300
    index1 = pp_data[(pp_data['Boiler_type'] == 'Super') & (pp_data['capacity'] < 300 & (pp_data['gce/kwh'] == 'null') )].index
    pp_data.loc[index1, 'gce/kwh'] = 308
    index1 = pp_data[(pp_data['Boiler_type'] == 'Subcritical') & (pp_data['capacity'] >= 300 & (pp_data['gce/kwh'] == 'null') )].index
    pp_data.loc[index1, 'gce/kwh'] = 314
    index1 = pp_data[(pp_data['gce/kwh'] == 'null') | (pp_data['gce/kwh'] == np.nan)].index
    pp_data.loc[index1, 'gce/kwh'] = 323
    
    key_info = ['NAME','PROV', 'YEAR', 'status_201707','latitude','Longitude','HOURS','gce/kwh','Boiler_type']
    pp_group = pp_data.groupby(by=key_info)    
    pp_group_sum = pp_group.sum()
    pp_tmp = pp_group_sum.loc[:,'capacity']
    pp = pp_tmp.reset_index()
    # pp['ID'] = [x for x in range(len(pp))] 
    pp = pp[pp['capacity'] >= 10]
    
    # pp_before2020 = pp[pp['YEAR']<=2020]
    # pp_before2020['capacity'].sum()
    
    # fill hours
    pp['HOURS'] = pp['HOURS'].replace('null', 4611.83) # source Zhangning
    pp = pp.rename(columns={'YEAR':'year',
                       'latitude':'lon',
                       'Longitude':'lat',
                       'HOURS':'hours',
                       'gce/kwh': 'CoalUnit_g/kwh',
                       'capacity':'capacity_MW',
                       'PROV':'prov'})
    # pp = pp.set_index(pp['ID'])

    # ========== 2. produce input pp dataset =========================
    # 2.1 calculate pp in which grid cell
    resolution = 20 # km
    pp['pixelX'] = round((pp['lon'] - 70) / 0.01 / resolution)
    pp['pixelY'] = round((55 - pp['lat']) / 0.01 / resolution)
    
    # 2.2 find the nearest carbon storage point and calculate the distance
    ccs_point = pd.read_excel('input/RawData/ccs_clusters.xlsx')
    for p in pp.index:
        lon_p = pp.loc[p, 'lon']
        lat_p = pp.loc[p, 'lat']
    ...
    # 2.4 others
    pp['Capacity_kw'] = pp['capacity_MW'] * 1000
    pp['E2020'] = pp.apply(lambda x: cal_emiss_yr(0,0,0,0,0,1,x['Capacity_kw'], x['hours'],0,0,x['CoalUnit_g/kwh'], x['year'], 2020), axis=1)
    pp['option'] = 'PP'
    pp['take'] = 0.
    
    # ===== 3. read coal plants from GEM ==========
    pp_lhr_st2020 = pp[pp['year'] <= 2020]  
    pp_lhr_st2020['E2020'] = pp_lhr_st2020.apply(lambda x: cal_emiss_yr(0,0,0,0,0,1,x['Capacity_kw'], x['hours'],0,0,x['CoalUnit_g/kwh'], x['year'], 2020), axis=1)
    pp_lhr_st2020['ID'] = [x for x in range(len(pp_lhr_st2020))]
    
    pp_gem = pretreat_coalPlant_GEM()
    pp_gem_lt2020 = pp_gem[pp_gem['year'] > 2020]
    pp_gem_lt2020['ID'] = [x for x in range(len(pp_lhr_st2020), len(pp_lhr_st2020) + len(pp_gem_lt2020))]
    # ======== 3.save results ============
    pick = ['ID', 'hours', 'Capacity_kw', 'CoalUnit_g/kwh',
            'lon', 'lat', 'year', 'pixelX','pixelY','ccsDis_km',
            'option','E2020','take', 'pp', 'flex','be','ccs','retire','prov']   
    
    future_pp = pp_gem_lt2020[pick] 
    current_pp = pp_lhr_st2020[pick]
    pp_out = pd.concat([current_pp, future_pp], axis=0)
    
    pp_out = pp_out[(pp_out['pixelX'] >= 0) & (pp_out['pixelX'] <= 350) & (pp_out['pixelY'] >= 0) & (pp_out['pixelY'] <= 200)]
    
    pp_out['ID'] = [x for x in range(len(pp_out))] 
    pp_out.index = pp_out['ID']
    ...
    pp_out['yrEmiss_10_6t'] = pp_out.apply(lambda x: cal_emiss_yr(x['be'][0], 0, x['ccs'][0], 0,
                            x['retire'][0], x['cf'], x['Capacity_kw'], x['hours'], x['take'],
                            0, x['CoalUnit_g/kwh'], x['year'], 2020) / pow(10,6), axis=1)
    
    pp_out['yrEmiss_10_6t'] = pp_out.apply(lambda x: cal_emiss_yr(0, 0, 0, 0,
                            0,1, x['Capacity_kw'], x['hours'], 0,
                            0, x['CoalUnit_g/kwh'], x['year'], 2020) / pow(10,6), axis=1)
    ...
    with open('input/CoalPlants/powerplants_2020_ppmax.pkl','wb') as f:
        pickle.dump(pp_dict1, f)
        
    # save a csv file
    pp_out1 = pp_out.drop('pos', axis=1)
    # pp_out1.to_csv('input/pp_all.csv', index=0)
    # pp_out1.to_csv('output/pp_all_2020.csv',index=0)
    
    pp_out1.to_csv('input/pp_all_ppmax.csv', index=0)
    pp_out1.to_csv('output/pp_all_2020_ppmax.csv',index=0)
    
    print('finish pretreatment')
    print('total number of coal plants: %.f' % len(pp_out1))
